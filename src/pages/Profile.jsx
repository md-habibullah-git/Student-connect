...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(usersRef, (snapshot) => {
      ...
      if (userId) {
        ...
        // 🛠️ Without touching the design, only the searching capability was increased (.includes)
        const matched = activeProfiles.filter(p => {
          ...
          const idNoField = String(p.idNo || '').toLowerCase().trim(); // ID No field
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', justifyContent: 'center', alignItems: 'center', padding: '25px 40px', boxSizing: 'border-box', backgroundColor: 'var(--bg)', minHeight: '90vh' }}>
      ...
      {profiles.map(user => {
        ...
        return (
          <div key={user.id} style={{ width: '100%', maxWidth: '750px', backgroundColor: 'var(--bg)', borderRadius: '20px', boxShadow: 'var(--shadow)', overflow: 'hidden', display: 'flex', flexWrap: 'wrap', border: '1px solid var(--border)', position: 'relative' }}>
            ...
            <div style={{ width: '270px', flexShrink: 0, background: 'linear-gradient(180deg, #022340 0%, #053c69 50%, #011627 100%)', padding: '15px 3px 25px 3px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'space-around', textAlign: 'center', position: 'relative', boxSizing: 'border-box' }}>
            ...
            <div style={{ flex: 1.3, minWidth: '280px', padding: '15px 25px 12px 25px', display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: '6px', backgroundColor: 'var(--code-bg)', color: 'var(--text)', overflow: 'hidden' }}>
              {[
                ...
                { label: "Persona Email:", value: user.email || 'N/A', icon: <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>, bg: '#f3e5f5', color: '#6a1b9a' }
              ].map((item, idx) => (
                <div key={idx} style={{ display: 'flex', alignItems: 'center', backgroundColor: 'var(--bg)', padding: '5px 12px', borderRadius: '12px', border: '1px solid var(--border)', boxShadow: '0 2px 4px rgba(0,0,0,0.01)' }}>
                  <div style={{ width: '28px', height: '28px', borderRadius: '8px', backgroundColor: item.bg, color: item.color, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginRight: '12px' }}>
                    {item.icon}
                  </div>
                  <span style={{ width: '105px', color: 'var(--text)', fontSize: '13px', fontWeight: '600' }}>{item.label}</span>
                  <div style={{ width: '1px', height: '14px', backgroundColor: 'var(--border)', marginRight: '12px' }}></div>
                  <strong style={{ color: item.isCritical ? '#d9383a' : 'var(--text-h)', fontSize: '13px', fontWeight: '700', wordBreak: 'break-all' }}>{item.value}</strong>
                </div>
              ))}
