...
// ✅ Audio Volume Boost Helper — GlobalChat-এর মতো
...
export default function PersonalChat() {
  ...
  useEffect(() => {
    if (inCall) {
      ...
      if (remoteAudioRef.current && remoteStreamRef.current) {
        ...
        // ✅ Audio Volume Boost — Helper function ব্যবহার
  ...
  const initiateCall = async (callType = 'video') => {
    try {
      cleanupCallLocally();
      callHistorySavedRef.current = false; // ✅ Caller — missed call save করবে
  ...
  const answerIncomingCall = async () => {
    try {
      cleanupCallLocally();
      callHistorySavedRef.current = false; // ✅ Receiver — received call save করবে
      callAnsweredTimeRef.current = null;
